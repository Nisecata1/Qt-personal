...
- `说明.md`：当前目录的构建、替换和兼容说明。

说明：仓库不再保留 `scrcpy-server~new`、`scrcpy-server~new2`、`scrcpy-server.zip` 这类本地备份或中间产物；如需归档，请保存在仓库外部。
...
- 默认设备点击仍走触屏注入；只有当客户端显式发送 `POINTER_ID_MOUSE` 时，server 才按 Android 鼠标事件注入
