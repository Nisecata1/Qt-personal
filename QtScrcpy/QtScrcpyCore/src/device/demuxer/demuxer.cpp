...
void Demuxer::run()
{
    ...
    m_parser = Q_NULLPTR;
    m_bitRateWindowElapsed.invalidate();
    m_bitRateWindowBytes = 0;
...
bool Demuxer::processFrame(AVPacket *packet)
{
    ...
    if (!m_bitRateWindowElapsed.isValid()) {
        m_bitRateWindowElapsed.start();
    }
    m_bitRateWindowBytes += static_cast<quint64>(qMax(0, packet->size));
    const qint64 elapsedMs = m_bitRateWindowElapsed.elapsed();
    if (elapsedMs >= 1000) {
        const quint64 bitRate = elapsedMs > 0
            ? (m_bitRateWindowBytes * 8ULL * 1000ULL) / static_cast<quint64>(elapsedMs)
            : 0ULL;
        emit updateBitRate(bitRate);
        m_bitRateWindowBytes = 0;
        m_bitRateWindowElapsed.restart();
    }
