...
void Recorder::setLastError(const QString &error)
{
    QMutexLocker locker(&m_mutex);
    m_lastError = error;
}
...
bool Recorder::open()
{
    ...
    if (!inputCodec) {
        setLastError(QStringLiteral("H.264 decoder not found."));
    ...
    if (!format) {
        setLastError(QStringLiteral("Could not find muxer for format: %1").arg(formatName));
    ...
    if (!m_formatCtx) {
        setLastError(QStringLiteral("Could not allocate output context."));
    ...
    if (!outStream) {
        setLastError(QStringLiteral("Could not allocate output stream."));
    ...
    if (ret < 0) {
        ...
        setLastError(QStringLiteral("Failed to open output file: %1 (%2)").arg(QString::fromUtf8(errorbuf), m_fileName));
...
void Recorder::close()
{
    if (Q_NULLPTR != m_formatCtx) {
        if (m_headerWritten) {
            ...
            if (ret < 0) {
                setLastError(QStringLiteral("Failed to write trailer to %1").arg(m_fileName));
            ...
            setLastError(QStringLiteral("The recorded file is empty: %1").arg(m_fileName));
            m_failed = true;
        }
...
{
    if (!m_formatCtx) {
        setLastError(QStringLiteral("Recorder output context is not open."));
        return false;
    }

    if (!m_headerWritten) {
        if (packet->pts != AV_NOPTS_VALUE) {
            setLastError(QStringLiteral("The first packet is not a config packet."));
    ...
    const int ret = av_write_frame(m_formatCtx, packet);
    if (ret < 0) {
        char errorbuf[255] = { 0 };
        av_strerror(ret, errorbuf, 254);
        setLastError(QStringLiteral("Could not record packet: %1").arg(QString::fromUtf8(errorbuf)));
        return false;
    }
    return true;
}
...
bool Recorder::recorderWriteHeader(const AVPacket *packet)
{
    if (!m_formatCtx || m_formatCtx->nb_streams <= 0) {
        setLastError(QStringLiteral("Recorder output stream is not initialized."));
        return false;
    }
    ...
    if (!extradata) {
        setLastError(QStringLiteral("Cannot allocate extradata."));
    ...
    if (ret < 0) {
        char errorbuf[255] = { 0 };
        av_strerror(ret, errorbuf, 254);
        setLastError(QStringLiteral("Failed to write recorder header: %1").arg(QString::fromUtf8(errorbuf)));
...
{
    if (!m_formatCtx) {
        setLastError(QStringLiteral("Recorder is not open."));
        return false;
    }
    start();
    return true;
...
bool Recorder::push(const AVPacket *packet)
{
    ...
    if (!rec) {
        m_lastError = QStringLiteral("Could not clone packet for recorder queue.");
    }
