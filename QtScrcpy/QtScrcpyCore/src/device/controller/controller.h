...
class Controller : public QObject
{
    Q_OBJECT
public:
    Controller(std::function<qint64(const QByteArray&)> sendData, const QString &serial, QString gameScript = "",
               QObject *parent = Q_NULLPTR, std::function<qint64()> pendingControlBytes = nullptr);
    virtual ~Controller();

    void postControlMsg(ControlMsg *controlMsg);
    void postControlMsg(ControlMsg *controlMsg, int priority);
    ...
    // for input convert
    bool sendControlMsgImmediately(ControlMsg *controlMsg);
    qint64 pendingControlBytes() const;
    ...
    void setDeviceClipboardText(QString &text, bool paste = true);
    void clipboardPaste();
...
private:
    ...
    std::function<qint64()> m_pendingControlBytes = nullptr;
    QString m_serial;
};
