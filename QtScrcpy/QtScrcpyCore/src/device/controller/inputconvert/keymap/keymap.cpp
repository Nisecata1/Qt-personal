    ...
    if (rootObj.contains("keyMapNodes") && rootObj.value("keyMapNodes").isArray()) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            case KeyMap::KMT_CLICK: {
                ...
                // 【在这里插入代码】读取 alwaysActive 属性
                // getItemBool 是该文件底部定义好的辅助函数，很方便
                keyMapNode.alwaysActive = getItemBool(node, "alwaysActive"); 
            ...
            case KeyMap::KMT_STEER_WHEEL: {
                ...
                // --- 【新增代码 START】 ---
                // 允许方向盘按键常驻（即使切出鼠标，松开 WASD 依然能触发停止移动）
                keyMapNode.alwaysActive = getItemBool(node, "alwaysActive"); 
                // --- 【新增代码 END】 ---
            ...
            case KeyMap::KMT_ANDROID_KEY: {
                ...
                // 建议： 同样的操作也可以加在 KMT_ANDROID_KEY (安卓按键) 的分支里（大概在第 280 行左右），这样你映射的音量键、返回键等物理按键也能常驻生效。
                keyMapNode.alwaysActive = getItemBool(node, "alwaysActive");
