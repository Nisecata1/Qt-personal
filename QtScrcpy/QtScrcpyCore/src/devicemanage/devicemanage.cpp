...
void DeviceManage::onDeviceConnected(bool success, const QString &serial, const QString &deviceName, const QSize &size, int initialOrientation)
{
    emit deviceConnected(success, serial, deviceName, size, initialOrientation);
    if (!success) {
        removeDevice(serial);
    }
}
